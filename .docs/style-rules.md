...
- Every top-level section heading is an `h2`. That means chapter leaves (`Chapter`) and every section that reads as a top-level division of the story: the in-chapter artifact sections a reader jumps to from the rail (`ArtifactSection` with `headingLevel={2}`) and the self-tiled artifact modules that carry their own `h2` (`EngineAudit`, `JourneyMap`, `IaFlow`, and peers). Every one of these wears the single shared display heading, `sectionHeading` in `Chapter.tsx`, so the look is defined once and no section title can drift; a nested subsection (`ArtifactSection` with `headingLevel={3}`) steps down one size in the same family via `subsectionHeading`.
- Headings nested inside a section step down in order — `h3`, then `h4` — and never skip a level. A content heading that sits under a section (a scorecard title, a persona card, a roadmap item) is an `h3`, not a second `h2`.
- Heading tags are semantic and independent of visual size: the class list carries the size, the tag carries the meaning. Never promote or demote a tag to reach for a font — headings already carry the display face at every level. But a section title never wears the small uppercase eyebrow style. Every top-level section title takes the shared `sectionHeading` display line and every nested one takes `subsectionHeading`; the uppercase tracked eyebrow is reserved for captions and kickers (§3), not for section headings.
