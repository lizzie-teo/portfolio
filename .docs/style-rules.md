...
- Chapters open on a dark leaf (`Chapter`): a full-screen title page in the `--leaf` / `--leaf-foreground` tokens holding only the small uppercase chapter title and its lede — the chapter's one speakable takeaway — like the dark divider page of a book. The leaf is sticky and the chapter's single white sheet — one viewport-wide, rounded-top holder per chapter (page background, `shadow-elevated`) carrying all of the chapter's content — slides up and covers it under native scroll; the cover is reader-driven positioning, not an animation, so it needs no reduced-motion variant and never hijacks scroll. Both leaf and sheet bleed to the viewport edges at every width and re-centre their content on the shell's container gutters; the floating rail overlays them. Leaf tokens live in `theme.css`: ink in the neutral shell, the project's brand hue pushed toward ink inside its scope (contrast with `--leaf-foreground` recorded at the token). Leaves carry no dividers and no numbering; a leaf's lede must be a conclusion, not a topic label, and jumping to a chapter anchor lands the reader on its leaf as arrival feedback.
...
- Wide desktop (`xl:` and up) uses the "On this page" rail (`CaseStudyRail`), a fixed overlay floating at the top right with scroll-spied `aria-current` states — it rides above both the dark chapter leaves and the white content sheets, with `shadow-card` on the tiles so they read as a layer on either surface. The rail floats visually but owns a dedicated lane: content containers widen their right gutter from `xl` up (`xl:pr-64 2xl:pr-72`) so no content ever runs beneath it. The rail is a stack of chapter blocks: every chapter is a rounded tile, and the chapter the reader is inside expands onto the `--rail-tile-active` pair (tile, foreground; the shell falls back to `--primary` / white), carrying its subsection links within it, while the rest sit collapsed as quick-jump tiles on the `--rail-tile` trio (tile, foreground, hover), which falls back to the quiet secondary panel. Project scopes may re-tint the whole set — Healthdirect runs mid-teal default tiles with a coral `#e37778` active tile carrying white labels. Position is expressed by which block is open — no numbering, no progress hardware. The expansion is a repeated in-flow interaction: Motion layout animation at the `fast` token, opacity-only under reduced motion. Labels stay typographic and Title Case throughout — chapter anchor links and subsection links alike, each rendered as a semibold `h4`-wrapped link so it carries the display face (uppercase is reserved for the on-page chapter eyebrows, not navigation). Wayfinding numbers appear nowhere in headings or nav links — chapter arrival feedback is the leaf itself; the chapter pill's progress fraction is the one remaining numeric element.
