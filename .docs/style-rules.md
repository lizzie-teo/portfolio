...
- The reveal is part of the design: a home card rests neutral and the product's colour arrives on hover, so the grid reads as one calm plane until the reader reaches for something. `LoFiProjectCard` derives its halftone ink from the project's payoff panel and floods that panel behind the client's mark on hover. **Those hues are artwork scene constants, not tokens** (see the carve-out below): the card paints them, it does not adopt the case study's theme. Token theming still waits for the case study — entering it is what re-tints `--primary`, `--border`, the surfaces, and everything else in the contract above. Keep the two apart. Colour on a card is a picture of where the link goes; colour in the shell is the product's air, and the home page never breathes it.
...
- **The home grid is typographic, not pictorial.** `LoFiProjectCard` renders no image at all: a flat paper plate carrying the project title, an `IndustryGlyph`, and a foot rule. Colour and imagery arrive only in the hover payoff, where the plate floods the project's panel colour behind a reversed client mark. So the grid's calm comes from type and one shared plate, and the reach for a card is what pays out a picture. A case study may still preserve its own native product, brand, or campaign visual language inside the portfolio frame.
...
- Page sections use `bg-background`, except where a section needs to read as a different plane rather than a different area of the same one. The home page is the worked case: its shell sits on `bg-secondary`, and the work section is a full-bleed `bg-grout text-grout-foreground` band — one step deeper than the leaf ink its cards use, so the cards read as tiles resting on a ground, the same plane-and-tile relationship the case studies run on. The band carries its own colour scheme rather than following the shell's, so it holds in both tones.
- **A tonal step is the edge; do not draw a rule as well.** Where a full-bleed band changes colour against the page, `border-y border-border` adds nothing at the seam and shows itself the moment the neighbouring section inverts, reading as a stray line across the page. Hairlines are for dividing content that shares a surface.
...
- Anchor navigation must move keyboard focus meaningfully, and it lands against the leaf pin line rather than a header offset. `SiteHeader` is a masthead in normal flow that scrolls away, so there is no sticky chrome to subtract; what does constrain the landing is `CollapsingLeaf`, whose target sits about 1cm above its pin line — cross that by a hair and the leaf pins flush and starts collapsing instead of resting. Because a long glide passes through sections that collapse and reveal on the way, a landing computed once at click time is stale on arrival, so `useAnchorScroll` recomputes the live target every frame and homes to it. Use that hook for every in-page jump rather than a fixed `scrollTo` with an offset constant.
...
- The heading tokens hold a deliberate size gap so level always reads from size: `leafHeading` (chapter-opener `h2`) `clamp(1.75rem,3.6vw,3.25rem)` (28 → 52px); `sectionHeading` (in-flow `h2`) `text-2xl md:text-3xl lg:text-4xl` (24 → 36px); `subsectionHeading` (`h3`) `text-lg md:text-xl lg:text-2xl` (18 → 24px) — h3 two steps below the in-flow h2 at desktop, so an `h3` never crowds the `h2` above it; `minorHeading` (`h4`) `text-base md:text-lg` (16 → 18px), one step below the h3 and the deepest the outline goes. Adjust a gap only by editing these tokens, never by hand-sizing an individual heading.
