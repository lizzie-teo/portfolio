/* ============================================================================
   ...
   USAGE
     mountScrollWorld(document.getElementById('world'), {
       ...
       crossfade: 0.12,   // DEFAULT seam width (viewport-heights) at every joint;
                          // per-joint overrides live on the sections (`seam`, below)
       sections: [
         { id, label, still, poster, posterMobile, clip, clipMobile, accent,
           ...
           seam: 0.22,    // optional per-joint override of `crossfade` — the width, in
                          // viewport-heights of scroll, of THE SEAM THIS SECTION ARRIVES
                          // ON (the joint between the previous clip and this one). It is
                          // a property of the joint, so it is owned by the incoming
                          // scene; section 0 has no leading joint and ignores it.
                          // Generated clips match their predecessor by wildly different
                          // amounts, so this is per-joint on purpose: a frame-adjacent
                          // joint wants ~0.03 (any wider and you see the same take twice)
                          // and a genuine content mismatch wants 0.25+.
                          // See THE SEAM below for what the width actually drives.
   ...
   THE SEAM (how one clip becomes the next)
     A generated clip re-draws its predecessor's last frame rather than truly
     continuing it, so every joint is a small discontinuity and no two joints are
     equally bad. The seam treatment is therefore per-joint (`seam`, above) and it
     is three coordinated moves, all derived from that one width:

     1. OVERLAPPED PLAYBACK. The outgoing clip's scroll range is extended to reach
        `seam` past the nominal boundary, so through the seam BOTH clips are still
        scrubbing — the outgoing playing its final frames while the incoming plays
        its first. A dissolve between two moving frames reads as travel; a dissolve
        onto a frozen frame reads as a stall, which is the thing being fixed.
        Nominal starts/ends are untouched, so copy timing, the segment chain and
        the total scroll length are all unchanged by any seam width.
     2. THE PUSH. The outgoing keeps travelling as it leaves: a small forward scale
        (from the scene's own vanishing point, set in CSS `transform-origin`) that
        grows across the seam. Removed under prefers-reduced-motion.
     3. THE THINNING. The incoming rises from slightly transparent, so at the heart
        of the seam neither layer fully covers the page and a few percent of the
        page background shows through both. The ink thins where the joint is
        loosest and the sheet underneath is the one thing every scene shares.
        Opacity only, so it survives reduced motion.

     Stacking is static: earlier segments sit above later ones, so an outgoing clip
     dissolves away to reveal the incoming already running underneath. (The previous
     build promoted the INCOMING to the top at full opacity exactly at the boundary,
     which occluded the outgoing and made every joint a one-pixel cut no matter what
     `crossfade` said.) At most two scenes are ever painted.
...
function mountScrollWorld(container, config) {
  ...
  // Default seam width (vh of scroll). Per-joint overrides ride on the sections as
  // `seam` — see THE SEAM above. Kept at the old global's value so a config that
  // sets nothing behaves exactly as before.
  const SEAM_DEFAULT = (config.crossfade != null) ? config.crossfade : 0.12;
  // Derived from each joint's own width, never configured separately: the looser the
  // joint, the more forward travel and the more paper it needs. Both are capped so a
  // very wide seam can't turn into a zoom or a wash.
  const SEAM_PUSH = 0.16, SEAM_PUSH_MAX = 0.045;   // extra scale on the outgoing clip
  const SEAM_THIN = 1.25, SEAM_THIN_MAX = 0.36;    // how transparent the incoming starts
  ...
  SECTIONS.forEach((s, i) => {
    // `seam` is the width of the joint this segment ARRIVES ON, so it is owned by the
    // incoming segment. Segment 0 has no leading joint; its value is forced to 0 below.
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still,
                   poster: s.poster, posterM: s.posterMobile,
                   accent: s.accent, w: s.scroll || DIVE_W, linger: s.linger || 0,
                   seamW: (s.seam != null) ? s.seam : SEAM_DEFAULT };
    SEGMENTS.push(dive);
    s._seg = dive;
    ...
    if (i < N - 1 && CONNECTORS[i]) {
      // A connector is generated to start on its predecessor's actual last frame, so
      // its own leading joint is the well-matched kind and takes the default width.
      // The joint the connector hands off ON is the next section's `seam`.
      SEGMENTS.push({ kind: 'conn', si: i, clip: CONNECTORS[i], clipM: CONNECTORS_M[i],
                      ...
                      accent: SECTIONS[i + 1].accent, w: CONN_W, seamW: SEAM_DEFAULT });
    }
  });
  ...
  SEGMENTS.forEach((s, i) => {
    ...
    // STATIC stacking, earlier segments on top: an outgoing clip dissolves away to
    // reveal the incoming one already running underneath. Written once, never per
    // frame — the seam is expressed entirely in opacity, which is the only way a
    // dissolve can actually be seen. (See THE SEAM in the header.)
    scene.style.zIndex = String(200 - i);
  ...
  // Seam dissolve curve: smoothstep applied twice. Softer than smoothstep at both
  // ends (the outgoing leaves reluctantly and the incoming arrives without a lurch)
  // and steeper through the middle, which is where the double exposure of two
  // mismatched drawings is most obvious — so the seam spends less time there.
  const dissolve = x => smooth(smooth(x));
  ...
  function layout() {
    ...
    totalW = off;
    // Seam geometry, derived after the chain is laid out. The seam scales with `wf`
    // along with everything else, so it stays the same FRACTION of a leg (and so of
    // the clip) on a phone as on a desktop.
    // `playEnd` reaches past the nominal boundary into the successor's leading seam:
    // that overlap is what keeps the outgoing clip scrubbing while it dissolves.
    // Nominal start/end are untouched, so copy timing, the segment chain and the
    // total scroll height are all independent of any seam width.
    SEGMENTS.forEach((s, i) => {
      const w = (i === 0) ? 0 : Math.max(0, s.seamW);            // segment 0 has no leading joint
      s.seam = w * wf * vh;
      s.push = reduce ? 0 : Math.min(SEAM_PUSH_MAX, w * SEAM_PUSH);
      s.thin = Math.min(SEAM_THIN_MAX, w * SEAM_THIN);
      const next = SEGMENTS[i + 1];
      s.playEnd = next ? next.start + Math.max(0, next.seamW) * wf * vh : s.end;
    });
  ...
  function read() {
    ...
    for (let i = 0; i < NSEG; i++) {
      ...
      // Playback runs to playEnd, which reaches into the next segment's leading seam,
      // so this clip is STILL SCRUBBING while it dissolves away over its successor.
      const local = clamp((y - s.start) / (s.playEnd - s.start), 0, 1);
      s.target = s.linger ? lingerEase(local, s.linger) : local;

      // At most two scenes are painted: the one you are in, and — while you are
      // inside a seam — the one you just left, sitting above it and dissolving.
      let op = 0, push = 1;
      if (i === ci) {
        op = 1;
        // The thinning: rise from slightly transparent across the seam so the page
        // shows through both layers at its heart. Hidden early under the outgoing
        // clip, which is still opaque there.
        if (s.seam > 0 && y < s.start + s.seam) {
          op = 1 - s.thin * (1 - clamp((y - s.start) / s.seam));
        }
      } else if (i === ci - 1) {
        const j = SEGMENTS[ci];                       // the joint belongs to the incoming
        if (j.seam > 0) {
          const r = clamp((y - j.start) / j.seam);
          op = dissolve(1 - r);
          // …and it keeps travelling as it goes. The ramp ACCELERATES (r², an
          // ease-in, which is what an exit takes) so almost all of the travel
          // happens in the back half, where the outgoing frame is nearly gone: the
          // last impression pulls away and lifts off rather than drifting through
          // the busy middle of the dissolve, where extra movement only muddies the
          // two drawings against each other.
          push = 1 + j.push * r * r;
        }
      }
      s.el.style.opacity = op; s.visible = op > 0.001;
      // Only written when it changes: at every joint but one this is the identity
      // transform for the whole scene, and 60fps style writes compete with the
      // currentTime seek this route issues on the same frame.
      if (push !== s.pushCur) {
        s.pushCur = push;
        s.el.style.transform = push === 1 ? '' : `scale(${push.toFixed(4)})`;
      }
      if (!s.hasClip || !s.ready) {
        // The still is offset by ±2vw to balance the copy column, so it needs a
        // cover scale to sit under that offset. Reduced motion drops the RAMP (the
        // still must not creep as you scroll) but must keep a CONSTANT scale — at
        // exactly 1 the offset walked the artwork off the frame and left 2vw of bare
        // page down one edge, which is the permanent state in stills mode.
        const sc = reduce ? 1.06 : 1.03 + local * 0.14;
        s.img.style.transform = `translateX(${stageX - 2}vw) scale(${sc.toFixed(3)})`;
      }
    }

    // ── THE COPY WINDOW ─────────────────────────────────────────────────────
    // THE ONE DELIBERATE DIVERGENCE FROM UPSTREAM IN THIS FILE. Everything else
    // here is the vendored engine unedited so it can be diffed; this loop is
    // not, and the reason is that upstream's window is wrong for THIS footage.
    //
    // It used to peak every title at the MIDDLE of its own leg. That is right
    // for a scene whose subject sits in the middle of its clip, and wrong for
    // these: every leg is a take that ENDS on its arrival, so the middle of a
    // leg is travel and the landmark lands on the boundary. A title on the old
    // curve was brightest over the travel and already fading when the picture
    // it names arrived.
    //
    // The window below is the SAME RULE world.css writes out per chapter — 60%
    // of a scene's own leg to 52% of the next, holding flat across 13 → 87% of
    // that span — so a title is at full strength across its own arrival. It is
    // DERIVED rather than tabulated: the engine already knows every segment's
    // start and end, so this tracks any change to a `scroll:` weight for free,
    // where the stylesheet's copy of the same rule has to be recomputed by hand
    // (world.css, THE WAYMARKERS). If the two ever disagree, this one is right.
    //
    // WHERE IT APPLIES. Browsers with scroll-driven animations never see it:
    // world.css drives the same elements from `--sw-flight` and a CSS animation
    // outranks the inline opacity written here. This is the fallback path, and
    // it exists so that a browser without those animations gets the same
    // timing rather than upstream's.
    const HOLD_IN = 0.13, HOLD_OUT = 0.87;
    for (let i = 0; i < N; i++) {
      const seg = SECTIONS[i]._seg;
      const nextSeg = SECTIONS[i + 1] ? SECTIONS[i + 1]._seg : null;
      // Per-scene override, for the chapter whose sentence has to wait on a
      // picture. Kept in the config beside the copy it retimes; world.css holds
      // the matching exception under THE LONG HOLD and the two must agree.
      const win = SECTIONS[i].copyWindow;
      let ws, we, hin = HOLD_IN, hout = HOLD_OUT;
      if (i === 0) {
        // Already on screen at scroll 0, so it has no entry to give — only the
        // hold across its own arrival and the goodbye.
        ws = 0; hin = 0; hout = 0.92;
        we = nextSeg ? nextSeg.start + 0.52 * (nextSeg.end - nextSeg.start) : seg.end;
      } else if (i === N - 1) {
        // The sign-off rises once and never comes down: there is no tenth scene
        // for it to make way for.
        ws = seg.start; we = totalW * vh; hin = 0.2; hout = 1;
      } else {
        ws = seg.start + 0.6 * (seg.end - seg.start);
        we = nextSeg ? nextSeg.start + 0.52 * (nextSeg.end - nextSeg.start) : seg.end;
      }
      if (win) {
        // Fractions of the whole flight, so they are the same numbers the
        // stylesheet writes as percentages. `totalW` already carries
        // `scrollMobileFactor`, so one pair serves both breakpoints.
        ws = win.start * totalW * vh; we = win.end * totalW * vh;
        if (win.hold) { hin = win.hold[0]; hout = win.hold[1]; }
      }
      const t = (we > ws) ? (y - ws) / (we - ws) : 0;
      let cop;
      if (t <= 0) cop = (i === 0) ? 1 : 0;
      else if (t >= 1) cop = (i === N - 1) ? 1 : 0;
      else if (hin > 0 && t < hin) cop = smooth(t / hin);
      else if (hout < 1 && t > hout) cop = smooth((1 - t) / (1 - hout));
      else cop = 1;
...
function injectCSS() {
  ...
  /* DIVERGES FROM UPSTREAM BY ONE TOKEN: body clips x with \`clip\`, not
  // \`hidden\`. One hidden axis computes the other axis to \`auto\`, which makes
  // BODY A SCROLL CONTAINER — and a view timeline attaches to its subject's
  // nearest ancestor scroll container, so world.css's \`--sw-flight\` timeline
  // (declared on .sw-root) attached to body instead of the root scroller. Body
  // never scrolls (the window does), so every scroll-driven animation on the
  // page sat frozen at its 0% keyframe: the whole copy column at opacity 0.
  // \`clip\` clips without creating a scroll container. html keeps \`hidden\`:
  // the root's overflow is propagated to the viewport rather than making html
  // its own scroller, so it was never part of the problem. */
  html{margin:0;background:var(--sw-bg,#F5EDE0);overflow-x:hidden;}
  body{margin:0;background:var(--sw-bg,#F5EDE0);overflow-x:clip;}
  ...
  .sw-scene{position:absolute;inset:0;opacity:0;overflow:hidden;will-change:opacity,transform;
    /* The seam push scales from the footage's own vanishing point, not the box
       centre, so the forward travel runs along the same axis as the camera's.
       Matches .sw-scene__video's object-position; keep the two in step. */
  ...
  @media (max-width:860px){
    ...
    .sw-scene{transform-origin:50% 46%;}
    .sw-hint{bottom:calc(20px + env(safe-area-inset-bottom));}
    .sw-route{gap:16px;right:6px;} .sw-route__label{display:none;}
  }
  /* Portrait phones crop the clip hard — the source is 7:3 (1470x630), not 16:9,
     so a portrait box keeps only about a third of each frame's width. Keep the
     framing centred so the focal subject (which the camera dives toward) stays
     in view. world.css overrides this rule unlayered and carries the measured
     derivation; if that number moves, move this one with it. */
  @media (max-width:860px) and (orientation:portrait){
    .sw-scene__video,.sw-scene__still{object-position:center 44%;}
    .sw-scene{transform-origin:50% 44%;}
  }
...
// ESM export for the Next.js route (appended at install; the file above is the
// skill's portable original, kept byte-identical for easy upstream diffs).
