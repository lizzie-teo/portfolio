...
export function ChapterMarker({ chapters }: ChapterMarkerProps) {
  ...
  // active chapter changes but collapsible by hand afterwards. Mirrors the dock;
  // defaultExpanded is deliberately no longer read.
      ...
      <AnimatePresence>
        {open && active ? (
          <motion.nav
            ...
            className="chapter-dock-glass pointer-events-auto mb-3 max-h-[60vh] w-full overflow-y-auto rounded-2xl px-2 text-leaf-foreground sm:max-w-sm"
          >
            {/* Vertical padding lives on the inner list, not the scroll
                container: Chrome/Safari drop a scroll container's padding-bottom
                once you scroll to the end, which left the last chapter (Phase 2)
                flush against the card's bottom edge. As list padding it scrolls
                with the content and is always honoured. */}
                    ...
                    <div className="flex items-center gap-1">
                      <h4 className="min-w-0 flex-1">
                        <a
                          ...
                          className={`${linkFocus} flex min-h-12 items-center gap-2.5 rounded-lg px-3 text-sm font-semibold leading-snug text-leaf-foreground transition-colors hover:bg-black/10`}
                        >
                              ...
                              <li key={section.id}>
                                <h4>
                                  <a
                                    ...
                                    className={`${linkFocus} flex min-h-11 items-center rounded-md px-2.5 text-xs font-medium leading-snug transition-colors hover:bg-black/10 ${
                                      isSectionActive
                                        ? "text-leaf-foreground"
                                        : "text-leaf-foreground/70"
                                    }`}
                                  >
                                    {section.title}
                                  </a>
